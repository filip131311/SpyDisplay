...
//   3. Tsumego of the day: one of Cho Chikun's 900 elementary life-and-death
//      problems, a different one every local day (changes at midnight)
//
...
static const int SCREEN_COUNT = 4;  // must match SCREENS[] below
...
// Shared by the screens that only depend on the local date (countdown,
// tsumego): they need nothing but a valid clock and change at midnight.
static int midnightRefresh(Context& ctx) {
  if (ctx.now == 0) {
    // No valid clock yet: sync it from the server's Date header (this also
    // refreshes the cached star count as a side effect).
    Serial.println("RTC not set, syncing over Wi-Fi");
...
// --- Screen 3: tsumego of the day ------------------------------------------

// The 900 problems are compiled in (include/tsumego_data.h, generated by
// tools/gen_tsumego.py) in a pre-shuffled order; the local day number picks
// the entry, so the problem changes at midnight and repeats after 900 days.
static void tsumegoRender(LovyanGFX& g, const Context& ctx) {
  const int w = g.width(), h = g.height();
  long day = ctx.now ? localDay(ctx.now, TZ_OFFSET_S) : 0;
  Tsumego p = tsumegoAt(tsumegoIndexForDay(day));

  g.setTextDatum(top_center);
  g.setFont(&fonts::FreeSans12pt7b);
  g.setTextSize(1);
  g.drawString("Tsumego of the day", w / 2, 30);

  g.setFont(&fonts::FreeSansBold18pt7b);
  g.drawString(String("Cho Chikun, Elementary #") + p.number +
                   "   |   Black to play",
               w / 2, 62);

  const int top = 110, bottom = 75;  // header above, footer below
  drawTsumegoBoard(g, p, 40, top, w - 80, h - top - bottom, TFT_BLACK,
                   TFT_WHITE);

  String status = ctx.now ? formatLocal(ctx.now, "%Y-%m-%d")
                          : String("Clock not synced");
  status += batteryStatus("Battery", ctx.battery);
  drawFooter(g, status, 3, SCREEN_COUNT);
}

static const Screen SCREENS[] = {
    {"stars", starsRefresh, starsRender},
    {"countdown", midnightRefresh, countdownRender},
    {"card", cardRefresh, cardRender},
    {"tsumego", midnightRefresh, tsumegoRender},
};
